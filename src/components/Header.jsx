...
const Header = () => {
  ...
  const searchQuaryHandler = (e) => {
    if ((e?.key === 'Enter' || e === 'searchButton') && searchQuary?.length > 0) {
      navigate(`/searchResult/${searchQuary}`)
    }
  }
    ...
    <div className='sticky top-0 z-10 flex flex-row items-center justify-between h-14 px-4 md:px-5 bg-black dark:bg-black'>
      ...
      <div className='flex h-5 items-center'>
        ...
        <Link to='/' className='flex h-5 items-center'>
          <img className='h-full hidden dark md:block' src={mobileMenu ? ytLogoMobile : ytLogo} alt='youtube' />
          <img className='h-full md:hidden' src={ytLogoMobile} alt='logo' />

        </Link>
      </div>
      <div className='group flex items-center'>
        <div className='flex h-8 md:h-10 md:ml-10 md:pl-5 border border-[#303030] rounded-l-3xl group-focuce-within:border-blue-500 md:group-focus-within:ml-5 md:group-focus-whithin:pl-0'>
